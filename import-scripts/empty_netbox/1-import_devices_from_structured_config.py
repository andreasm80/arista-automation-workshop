...
def get_or_create_tenant(tenant_name, slug, group_name="dc1-production"):
...
def collect_ip_usage_and_prefixes():
    """Collect IPs for Anycast detection and prefixes for creation across all YAML files."""
    ip_usage = {}
    prefixes = {}
                ...
                try:
                    ...
                    for intf_type, intfs in [
                        ...
                        for intf_data in intfs:
                            ...
                            if ip_addr:
                                ...
                                vrf_name = intf_data.get("vrf", "vrf-dc1-prod-underlay")
                        ...
                        for seq in prefix_list.get("sequence_numbers", []):
                            ...
                            if "permit" in action:
                                ...
                                if prefix:
                                    ...
                                    if network_prefix:
                                        vrf_name = "vrf-dc1-prod-underlay"
                                        if vrf_name not in prefixes:
                                            prefixes[vrf_name] = set()
                                        prefixes[vrf_name].add(network_prefix)
                    ...
                    for route in config.get("static_routes", []):
                        ...
                        if prefix and prefix != "0.0.0.0/0":
                            network_prefix = ip_to_network_prefix(prefix)
                            if network_prefix:
                                vrf_name = route.get("vrf", "vrf-dc1-prod-underlay")
                                if vrf_name not in prefixes:
                                    prefixes[vrf_name] = set()
                                prefixes[vrf_name].add(network_prefix)
    ...
    print_regular(f"Detected Anycast IPs: {list(anycast_ips.keys())}")  # Debug output
...
def create_prefixes(prefixes, vrfs):
    ...
    for vrf_name, prefix_set in prefixes.items():
        vrf = vrfs.get(vrf_name, vrfs["vrf-dc1-prod-underlay"])
        tenant = vrf.tenant
...
def update_interface(device, intf_data, intf_type, anycast_ips, vrfs):
    ...
    if ip_addr:
        ...
        print_regular(f"Processing IP {ip_addr} for {intf_name}, is_anycast: {is_anycast}")  # Debug output

        # Check if this interface already has this IP assigned
        ...
        # Check all existing IPs with this address globally
        existing_ips = list(nb.ipam.ip_addresses.filter(address=ip_addr))

        if is_anycast or len(existing_ips) > 0:  # Fallback: Treat as Anycast if duplicates exist in NetBox
            # Try to reuse or update an existing Anycast IP
            for ip in existing_ips:
                if (ip.tenant and ip.tenant.id == tenant.id and
                    ...
                    if not ip.assigned_object or ip.assigned_object_id != interface.id:
                        ...
                        return
            # Create new Anycast IP if no suitable one exists
            try:
...
def update_management_interface(device, mgmt_intf_data, anycast_ips, vrfs):
    ...
    if new_ip_addr:
        ...
        vrf_name = mgmt_intf_data.get("vrf", "vrf-dc1-prod-underlay")
        vrf = vrfs.get(vrf_name, vrfs["vrf-dc1-prod-underlay"])
        tenant_name = "dc1-production-anycast" if is_anycast else "dc1-production-underlay"
        tenant = get_or_create_tenant(tenant_name, tenant_name.lower().replace(" ", "-"))
        description = f"{intf_name} IP for {device.name}"

        print_regular(f"Processing IP {new_ip_addr} for {intf_name}, is_anycast: {is_anycast}")  # Debug output
        ...
        if is_anycast or len(list(nb.ipam.ip_addresses.filter(address=new_ip_addr))) > 0:  # Fallback Anycast check
...
def verify_anycast_assignments(device, anycast_ips, vrfs):
    ...
    for intf in nb.dcim.interfaces.filter(device_id=device.id):
        ...
        if not os.path.exists(yaml_file):
            yaml_file = yaml_file.replace(".yml", ".yaml")
        if not os.path.exists(yaml_file):
            continue
...
def import_device_from_yaml(yaml_file):
    ...
    anycast_ips, prefixes = collect_ip_usage_and_prefixes()
    ...
    vrfs = {
        ...
        "vrf-dc1-prod-underlay": get_or_create_vrf("vrf-dc1-prod-underlay", "dc1-production-underlay", "dc1-production-underlay")
    }
    ...
    if new_ip:
        ...
        interface = nb.dcim.interfaces.get(device_id=device.id, name="Management0")  # Adjust if interface name differs
